...
module.exports = (input, file) => {
  ...
  const parser = {
    ...
    isAhead: function(tokenType, num = 1) {
      verify(tokenType);
      return this.input.length <= num ? false : this.input[num].tokenType === tokenType;
    },
    isAheadValue: function(value, num = 1) {
      return this.input.length <= num ? false : this.input[num].value === value;
    },
    getValue: function() {
  ...
  function die(msg) {
    throw {msg, position: parser.eatenInput[parser.eatenInput.length - 1].position, type: `PARSING`, file};
  }
  return parser;
}
