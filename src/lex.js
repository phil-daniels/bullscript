...
module.exports = (file, files) => {
  ...
  const getLastMatch = lexer.getLastMatch.bind(lexer);
  ...
  /**
   * If there are tokens between the -> and the next newline,
   * this is a single statement fn declaration, so we need to
   * add a statementend for the fn declaration
   */
  function convertFunctionDeclaration() {
    lexer;
    debug(`FUNCTION DECLARATION MODE`);
    create(`functiondeclarationstart`);
    const bookmark = lexer.eatenInput.length;
    lexUntil(`\r\n`, `\n`);
    if (lexer.eatenInput.length !== bookmark) {
      create(`statementend`);
    }
  }
